...
check_ubicacion_generica_ggmap_y_osm = function(inputArchivo = 'bases/ejemplo/ejemplo.csv',ubicaciones_genericas_ggmap_path = 'ubicaciones_genericas/ubicaciones_genericas_ggmap.csv',ubicaciones_genericas_osm_path = 'ubicaciones_genericas/ubicaciones_genericas_osm.csv',campos_genericos1=c("Localidad","Calle","Número"),campos_genericos2=c("Partido","Localidad","Calle","Número"),prefijo='ARGENTINA',id_column='IDEVENTOCASO',apikey  = readLines('apikey.txt'),write.it=T,verbose=T,timeout=5,texto_a_eliminar=c('*SIN DATO* (*SIN DATO*)','NULL'),lonlat_columns_ggmap=c('LON_RESIDENCIA','LAT_RESIDENCIA'),lonlat_columns_osm=c('LON_RESIDENCIA_OSM','LAT_RESIDENCIA_OSM'),invertir=F){
  ...
  loc_genericas = genera_loc_domicilios(data_ggmap,campos_genericos1,prefijo = prefijo,invertir=invertir)
  loc_genericas = c(loc_genericas,genera_loc_domicilios(data_ggmap,campos_genericos2,prefijo = prefijo),invertir=invertir)
  ...
  if(file.exists(ubicaciones_genericas_ggmap_path)){
    data_ggmap = check_ubicacion_generica_ggmap(data_ggmap,ubicaciones_genericas_ggmap,campos_genericos1,campos_genericos2,lonlat_columns_ggmap,texto_a_eliminar,verbose,invertir=invertir)
  }else{
    print('No existe archivo de ubicaciones genericas para GOOGLE')
  }
  if(file.exists(ubicaciones_genericas_osm_path)){
    data_osm = check_ubicacion_generica_osm(data_osm,data_ggmap,ubicaciones_genericas_osm,campos_genericos1,campos_genericos2,lonlat_columns_osm,texto_a_eliminar,verbose,invertir=invertir)
  }else{
