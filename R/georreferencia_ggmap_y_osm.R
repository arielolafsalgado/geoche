...
#' @export
georreferencia_ggmap_y_osm = function(inputArchivo = "bases/ejemplo/ejemplo.csv",campos_a_domicilio=c("Localidad","Calle","Número"),campos_a_domicilio2=c("Partido","Localidad","Calle","Número"),prefijo_domicilio='ARGENTINA',id_column='IDEVENTOCASO',apikey  = readLines('apikey.txt'),sep=',',write.it=T,verbose=T,timeout=5,invertir=F){
  d1 = georreferencia_ggmap(inputArchivo,campos_a_domicilio,prefijo_domicilio,apikey,sep,write.it,verbose,invertir=invertir)
  d2 = georreferencia_osm(inputArchivo,id_column,campos_a_domicilio,campos_a_domicilio2,prefijo_domicilio,sep,write.it,verbose,timeout,invertir=invertir)
